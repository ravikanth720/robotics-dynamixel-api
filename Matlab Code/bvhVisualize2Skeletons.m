function bvhVisualize2Skeletons(skelStructA, channelsA, skelStructB, channelsB, frameLength)
...
handleA = skelVisualise(channelsA(1, :), skelStructA);
handleB = skelVisualise(channelsB(1, :), skelStructB);
...
for i = 1:size(channelsA, 1)
  Y = skel2xyz(skelStructA, channelsA(i, :));
  minY1 = min([Y(:, 1); minY1]);
  minY2 = min([Y(:, 2); minY2]);
  minY3 = min([Y(:, 3); minY3]);
  maxY1 = max([Y(:, 1); maxY1]);
...
minY2 = zlim(1);
maxY2 = zlim(2);
for i = 1:size(channelsB, 1)
  Y = skel2xyz(skelStructB, channelsB(i, :));
  minY1 = min([Y(:, 1); minY1]);
  minY2 = min([Y(:, 2); minY2]);
  minY3 = min([Y(:, 3); minY3]);
  maxY1 = max([Y(:, 1); maxY1]);
...
% Play the motion
for j = 1:size(channelsA, 1)
  pause(frameLength)
  skelModify(handleA, channelsA(j, :), skelStructA);
  skelModify(handleB, channelsB(j, :), skelStructB);
end
